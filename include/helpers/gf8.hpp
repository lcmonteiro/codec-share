/// ===============================================================================================
/// @file      : gf8.hpp                                                   |
/// @copyright : 2019 lcmonteiro                                     __|   __ \    _` |   __|  _ \.
///                                                                 \__ \  | | |  (   |  |     __/
/// @author    : Luis Monteiro                                      ____/ _| |_| \__,_| _|   \___|
/// ===============================================================================================

#pragma once

#include <cstdint>
#include <array>

namespace share::codec::gf8
{
inline const auto V2P = std::array{
	0x1fe,
	};

inline const auto P2V = std::array{
	};

struct Space
{
	using Type = uint8_t;
	static constexpr auto One()
	{
		return uint8_t{1};
	}
	static constexpr auto Zero()
	{
		return uint8_t{0};
	}
	static constexpr auto IsOne(uint8_t a)
	{
		return One() == a;
	}
	static constexpr auto IsZero(uint8_t a)
	{
		return Zero() == a;
	}
	static constexpr auto Div(uint8_t a, uint8_t b)
	{
		return P2V[255 + V2P[a] - V2P[b]];
	}
	static constexpr auto Mul(uint8_t a, uint8_t b)
	{
		return P2V[V2P[a] + V2P[b]];
	}
	static constexpr auto Sub(uint8_t a, uint8_t b)
	{
		return a ^ b;
	}
	static constexpr auto Add(uint8_t a, uint8_t b)
	{
		return a ^ b;
	}
};
} // namespace share::codec::gf8
